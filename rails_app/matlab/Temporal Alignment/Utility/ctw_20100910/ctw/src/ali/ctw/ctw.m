...
while nIt < nItMa
    ...
    % before transformation, the sample has to be centralize
    if isKnl
        X1 = cenK(X10, W1);
        X2 = cenK(X20, W2);
    else
    ...
    Z2 = X2 * W2;
    if exist('b', 'var')
        parCca.egy = []; parCca.b = b;
    end
    [tmp1, tmp2, V1, V2] = cca(Z1, Z2, parCca);
    b = size(V1, 2);
...
its(nIt + 1 : end) = [];
if isKnl
    prom('m', 'kctw converged in %d steps\n', nIt);
else
    prom('m', 'ctw converged in %d steps\n', nIt);
end
