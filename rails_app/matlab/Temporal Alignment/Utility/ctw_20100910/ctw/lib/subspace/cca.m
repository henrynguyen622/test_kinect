function [Y1, Y2, V1, V2, v, me1, me2] = cca(X1, X2, par)
...
%     b     -  #diemnsion kept in the reduction
...
egy = ps(par, 'egy', []);
b = ps(par, 'b', []);
isDebg = psY(par, 'debg', 'y');
isDebg = 0; % changed to 0 for non-plot
...
if rank(C11) < d1 || rank(C22) < d2
    ...
    elseif strcmp(reg, 'cross')
        ...
        error(['unknown regularization algorithm: ' reg]);
    end
    prom('t', 'reg %.5f %.5f\n', lams(1), lams(2));
end
